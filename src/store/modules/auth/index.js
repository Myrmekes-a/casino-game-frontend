...
function getAuthErrorPayload(err) {
    if (err.response && err.response.data) {
        const d = err.response.data;
        const msg = (d.error && (d.error.message || d.error)) || d.message || (typeof d.error === 'string' ? d.error : null);
        if (msg) return { message: typeof msg === 'string' ? msg : (msg.message || String(msg)) };
    }
...
const actions = {
    async authGetUser({ getters, commit, dispatch }, data) {
        ...
        try {
            ...
            if(err.response && err.response.status === 401) { dispatch('authLogoutUser'); }
            dispatch('notificationShow', getAuthErrorPayload(err));
        }
        ...
        try {
            ...
            if(res.data.success) {
                if(res.data.step === 'captcha') {
                    ...
                    dispatch('modalsSetData', { challengeId: res.data.challengeId, captchaId: res.data.captchaId, captchaUrl: `${getBackendUrl()}/captcha/iframe?publicKey=476068BF-9607-4799-B53D-966BE98E2B81&data-exchange-blob=${res.data.captchaBlob}`, secureAuthenticationIntent: res.data.secureAuthenticationIntent });
