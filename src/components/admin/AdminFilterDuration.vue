<template>
    <div class="admin-filter-duration" v-bind:class="{ 'duration-open': adminDropdown === true }">
        <button v-on:click="adminToggleDropdown" class="button-toggle">
            <div class="button-value">
                Duration: <span>{{adminFilterDuration}} days</span>
            </div>
        ...
        <div class="duration-menu">
            <div class="menu-inner">
                <button v-on:click="adminSetButton('3')">3 days</button>
                <button v-on:click="adminSetButton('7')">7 days</button>
                <button v-on:click="adminSetButton('30')">30 days</button>
            </div>
        </div>
    </div>
</template>
...
<script>
  ...
  export default {
      ...
      methods: {
          ...mapActions(['adminSetFilterDuration']),
          adminToggleDropdown() {
              this.adminDropdown = !this.adminDropdown;
          },
          adminSetButton(value) {
              this.adminSetFilterDuration(value);
              this.adminToggleDropdown();
          }
      },
      computed: {
          ...mapGetters(['adminFilterDuration'])
      },
      created() {
          let self = this;
...
<style scoped>
  ...
  .admin-filter-duration button.button-toggle {
      width: 100%;
      height: 47px;
      display: flex;
  ...
  .admin-filter-duration.duration-open button.button-toggle svg {
      transform: rotate(180deg);
  }

  .admin-filter-duration button.button-toggle .button-value {
      font-size: 16px;
      font-weight: 600;
      color: #49687d;
  }

  .admin-filter-duration button.button-toggle .button-value span {
      color: #ffffff;
  }

  .admin-filter-duration .duration-menu {
      width: 100%;
      height: 0;
      position: absolute;
      top: 51px;
  ...
  .admin-filter-duration.duration-open .duration-menu {
      height: 147px;
  }

  .admin-filter-duration .menu-inner {
      width: 100%;
      padding: 3px;
      border-radius: 5px;
      background: #022038;
