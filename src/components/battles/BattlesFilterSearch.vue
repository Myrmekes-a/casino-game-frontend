<template>
    <div class="battles-filter-search">
        <input v-on:input="battlesSetFilterSearch(battlesSearch)" v-model="battlesSearch" type="text" placeholder="SEARCH FOR CASES..." />
...
<script>
    ...
    export default {
        ...
        created() {
            this.battlesSearch = this.battlesFilterSearch;
        }
    }
</script>
...
<style scoped>
    ...
    .battles-filter-search:before {
        ...
        left: 0;
        background: #0a273f;
        clip-path: polygon(8px 0, calc(100% - 9px) 0, 100% 25%, 100% 75%, calc(100% - 8px) 100%, 9px 100%, 0 75%, 0 25%);
        z-index: -1;
    }

    .battles-filter-search input {
        width: 100%;
        height: 100%;
        ...
        clip-path: polygon(10px 0, calc(100% - 8px) 0, 100% 25%, 100% 75%, calc(100% - 9px) 100%, 9px 100%, 0 75%, 0 25%);
    }

    .battles-filter-search input::placeholder {
        color: rgba(110, 149, 182, 0.5);
    }

    .battles-filter-search svg {
        position: absolute;
        top: 50%;
        left: 15px;
