    ...
    export default {
        ...
        computed: {
            ...
            chatGetGameLink() {
                ...
                if(
                    this.message.message.replace(/&#x2F;/g, '/').match(/(slotgemz\.com\/battles\/[0-9a-fA-F]{24})/) !== null ||
                    this.message.message.replace(/&#x2F;/g, '/').match(/(slotgemz\.com\/duels\?game=[0-9a-fA-F]{24})$/) !== null
                ) {
                    link = this.message.message.replace(/&#x2F;/g, '/').replace(/.*slotgemz\.com/, '');
                }
...
<style scoped>
    ...
    .chat-message-element button.button-game {
        ...
        padding: 1px;
        background: #01ad6f;
        clip-path: polygon(7px 0, calc(100% - 5px) 0, 100% 25%, 100% 75%, calc(100% - 6px) 100%, 6px 100%, 0 75%, 0 25%);
    }
