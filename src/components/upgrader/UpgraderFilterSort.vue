<template>
    <button 
        v-on:click="upgraderSetFilterSortCases(upgraderFilterSort === 'highest' ? 'lowest' : 'highest')" 
        class="upgrader-filter-sort" 
        v-bind:class="{ 'sort-lowest': upgraderFilterSort === 'Lowest' }"
    >
...
<style scoped>
    ...
    button.upgrader-filter-sort::before {
        content: '';
        width: 100%;
        height: 100%;
        position: absolute;
        ...
        left: 0;
        background: #092a45;
        clip-path: polygon(10px 0, calc(100% - 9px) 0, 100% 25%, 100% 75%, calc(100% - 10px) 100%, 9px 100%, 0 75%, 0 25%);
    }
    ...
    button.upgrader-filter-sort.sort-lowest .button-inner svg.toggle-icon {
        transform: rotate(180deg);
    }

    button.upgrader-filter-sort .inner-value {
        display: flex;
        align-items: center;
        font-size: 13px;
        font-weight: 600;
